...
class OrderController extends Controller
{
    ...
    //   $data=Order::where('created_by',$user->id)->with('items','items.measurements')->get();
    ...
    //         'air_mail.numeric'            => 'Air mail must be a number.',
    //         'air_mail.min'                => 'Air mail cannot be negative.',
    ...
    //         'customer_image.*.image'      => 'Each customer image must be a valid image.',
        
    //         'bill_book_copy.required'     => 'Bill book copy is required.',
    //         'bill_book_copy.*.mimes'      => 'Bill book copy must be a file of type: jpg, jpeg, png, webp, pdf.',
        
    //         'verified_video.*.mimes'      => 'Each verified video must be mp4, avi, or mov.',
    //         'verified_audio.*.file'       => 'Each verified audio must be a valid file.',
    ...
    //     $totalProductAmount = collect($validated['items'])
    //         ->sum(function ($item) {
    ...
    //     $pending     = max(0, $totalAmount - $paidAmount); // prevent negative
    ...
    //         // Customer Info
    //       $customerId = $prefix = $customerName = $customerEmail = null;
    ...
    //      // Define file types and folder mapping
    //         $fileTypes = [
    ...
    //             $files = $request->file($field); // get uploaded files
    //             if ($files) {
    //                 // Make sure it's always an array (single file => array with 1 element)
    //                 if (!is_array($files)) {
    //                     $files = [$files];
    //                 }
            
    //                 $paths = []; // store all uploaded paths
    ...
    //                 // Save all file paths as comma-separated string
    //                 OrderMultipleFile::create([
    //                     'order_id' => $order->id,
    //                     'file_type' => $field,
    //                     'file_path' => implode(',', $paths)  // comma-separated paths
    //                 ]);
    //             }
    //         }
    ...
    //             // 3. Ledger entry (credit = customer paid us)
    //             Ledger::insert([
    //                 'user_type'           => 'customer',
    //                 'customer_id'         => $customer->id,
    ...
    //             // 5. PaymentCollection entry
    //             //    is_ledger_added = 0, is_approve = 0 → pending admin approval
    //             //    (matches mobile app flow — admin approves later from web)
    ...
    //                 'is_ledger_added'   => 0,   // pending — admin will approve from web
    //                 'is_approve'        => 0,
    //                 'is_settled'        => 0,
    //                 'created_from'      => 'app',
    //                 'created_at'        => now(),
    ...
    //                 'items' => $order->items()->select('product_id','quantity','piece_price','total_price','status')->get(),
    //             ]
    //         ], 201);
